...
//     // res.user signed in ✅
//     // TODO: create/merge your Firestore user doc & run your gating here
//   } catch (err: any) {
//     if (err.code === "auth/account-exists-with-different-credential") {
//       // Same email already has another sign-in method; link it
...
//         // Ask the user to log in with email/password first, then:
//         // await signInWithEmailAndPassword(auth, email, password);
//         if (auth.currentUser) await linkWithCredential(auth.currentUser, pendingCred);
//       } else {
//         // If the existing method is another provider, start that flow then link
//       }
//     } else if (err.code === "auth/popup-blocked") {
//       await signInWithRedirect(auth, google); // iOS/Safari fallback
//     } else {
    ...
    try {
      ...
      if (e.code === "auth/account-exists-with-different-credential") {
        ...
        try {
          ...
          setErr(ERROR_MAP[e2.code] || "Google sign-in failed. Please try again.");
        }
      } else {
        setErr(ERROR_MAP[e.code] || "Google sign-in failed. Please try again.");
      }
  ...
  const handleResetPassword = async () => {
    ...
    try {
      ...
      if (error.code === "auth/invalid-email") {
        ...
        dispatch(showMessage("Unable to send reset email. Please try again later."));
      }
      console.error("Password Reset Error:", error);
    }
  };
  ...
  return (
    <main className="auth-page" role="main" aria-labelledby="login-title">
      <div className="auth-card">
        ...
        {err && (
          <div id="form-error" className="auth-alert" role="alert" aria-live="assertive">
            {err}
          </div>
        )}
        {submittedReset && (
        ...
        >
          ...
          <footer className="auth-footnote" aria-label="New user options">
            <span>New here?</span>{" "}
            <Link to="/request-access" className="auth-link">Request access</Link>
          </footer>
        </form>
      </div>
    </main>
