...
const CompanyGoalDetailsCard: React.FC<CompanyGoalDetailsCardProps> = ({
  ...
  useEffect(() => {
    if (goal.targetMode === "goalForAllAccounts" && allCompanyAccounts.length === 0) {
  ...
  const effectiveAccounts: CompanyAccountType[] = useMemo(() => {
    ...
    if (goal.targetMode === "goalForSelectedAccounts") return goal.accounts || [];
    if (goal.targetMode === "goalForSelectedUsers" && goal.usersIdsOfGoal?.length) {
      return allCompanyAccounts.filter((account) =>
        account.salesRouteNums?.some((route) => goal.usersIdsOfGoal!.includes(route))
      );
    }
    return [];
  ...
  const { total, submitted, percentage } = useMemo(() => {
    if (goal.targetMode === "goalForSelectedUsers" && goal.usersIdsOfGoal?.length) {
      ...
      const submittedCount = Array.from(uniqueSubmitters).filter(
        (uid) => goal.usersIdsOfGoal!.includes(uid as string)
      ).length;
      ...
      <div className="mobile-goal-box">
        ...
        <div className="mobile-main-box">
          <div className="mobile-content">
            {activeTab === 0 && (
              <>
                <div className="info-item title-bold"><strong>Title:</strong> {goal.goalTitle}</div>
                {onDelete && (
                  <div className="top-right-buttons">
                    <div className="goal-delete">
                      <CloseIcon className="delete-button" onClick={() => onDelete(goal.id)} fontSize="small" />
                    </div>
                    <Button variant="outlined" size="small" onClick={() => setIsEditModalOpen(true)}>Edit</Button>
                  </div>
                )}
                <div className="info-item description-label">
                  <div className="description-text"><strong>Description:</strong> {goal.goalDescription}</div>
                </div>
              </>
            )}
            {activeTab === 1 && (
              <>
                <div className="info-item"><strong>Start Date:</strong> {formatDate(goal.goalStartDate)}</div>
                <div className="info-item"><strong>End Date:</strong> {formatDate(goal.goalEndDate)}</div>
              </>
            )}
            {activeTab === 2 && (
              <>
                <div className="info-item"><strong>Metric:</strong> {goal.goalMetric}</div>
                <div className="info-item"><strong>Min number:</strong> {goal.goalValueMin}</div>
              </>
            )}
          </div>

          <Box px={2} pb={1}>
            <Typography variant="caption" color="textSecondary">{titleText}</Typography>
          ...
          <div className="mobile-accounts-tab">
            <button className={`file-tab ${expanded ? "active-tab" : ""}`} onClick={() => setExpanded(!expanded)}>
          ...
          <Collapse in={expanded} timeout="auto" unmountOnExit className="expanded-submissions">
            ...
            />
            <AccountTable accounts={filteredRows} navigate={navigate} height={isMobileScreen ? 300 : 60} />
          </Collapse>
        </div>
      </div>
    );
          ...
          <div className="info-header">
            <div className="info-title-row">
              ...
              {onDelete && (
                <div className="top-right-buttons">
                  <div className="goal-delete">
                    <CloseIcon fontSize="small" className="delete-button" onClick={() => onDelete(goal.id)} />
                  </div>
                  <Button variant="outlined" size="small" onClick={() => setIsEditModalOpen(true)}>Edit</Button>
                </div>
              )}
