...
export async function fetchAllAccountsFromFirestore(companyAcctsDocId: string ): Promise<CompanyAccountType[]> {
  ...
  try {
    ...
    if (!accountsSnapshot.exists()) {
      console.error("Accounts document not found for accountId:", companyAcctsDocId);
      return [];
    }
