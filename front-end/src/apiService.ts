...
export const fetchGalloAccounts = async (
  ...
  if (!response.ok)
    throw new Error(`Failed to fetch Gallo accounts: ${response.statusText}`);
  return response.json();
};
...
export const fetchCompanyAccounts = async (
  ...
  if (!accountsSnapshot.exists())
    throw new Error("No company accounts found in Firestore");
